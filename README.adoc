...
Note that each field could have defined a set of "possible_values" that will limit values to be exactly these.

===== Possible values weighted

Note that a weight (choose between 0 & 100 and whose sums of all weights must be 100) could also be defined
...
Example:
        "conditionals": {
            "always": "2 * increment_long + 56 - increment_int"
        }
...
Conditions must be equals (=) or superior (>) or inferior (<).
