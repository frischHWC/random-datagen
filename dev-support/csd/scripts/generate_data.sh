...
case $CMD in
 (gen_customer_hdfs_ozone_hive)
  ...
  (gen_ps_kafka_kudu)
    ...
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/public_service/sensor-data-model.json 10000 10 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} kafka kudu
     ret=$?
     if [ $ret -ne 0 ]; then
  ...
  (gen_weather_solr_kafka)
    ...
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/public_service/weather-model.json 10000 100 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} solr
     ret=$?
     if [ $ret -ne 0 ]; then
    ...
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/public_service/weather-sensor-model.json 1000 100 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} kafka
     ret=$?
     if [ $ret -ne 0 ]; then
  ...
  (gen_local_data)
    ...
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/industry/plant-model.json 10 1 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} csv
     ret=$?
     if [ $ret -ne 0 ]; then
       echo " Unable to generate data for plant-model.json"
       exit 1
     fi
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/industry/sensor-model.json 10 1 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} json
     ret=$?
     if [ $ret -ne 0 ]; then
       echo " Unable to generate data for sensor-model.json"
       exit 1
     fi
    ${PYTHON_COMMAND_INVOKER} ${CONF_DIR}/scripts/generate_data.py ${SERVER_PORT} /opt/cloudera/parcels/DATAGEN/models/industry/sensor-data-model.json 10 1 3600 ${TLS_ENABLED} ${ADMIN_USER} ${ADMIN_PASSWORD} csv
     ret=$?
     if [ $ret -ne 0 ]; then
       echo " Unable to generate data for sensor-data-model.json"
       exit 1
